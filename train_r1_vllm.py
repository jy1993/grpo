...
from transformers import Qwen2ForCausalLM, Qwen2Tokenizer, get_linear_schedule_with_warmup, get_constant_schedule_with_warmup, AdamW, StoppingCriteriaList
...
class Trainer(object):
	...
	def __init__(self, args):
		...
		# print(self.model_engine.device, self.ref_model_engine)
		...
		self.data_buffer = GRPOBuffer(pad_token_id=self.tokenizer.pad_token_id, eos_token_id=self.tokenizer.eos_token_id, assistant_token_id=77091, tokenizer=self.tokenizer, args=self.args)
	...
	def batch_compute_logps(self, model, input_ids, attention_mask, logits_to_keep):
		...
		for i in range(max(1, len(input_ids) // mini_batch_size)):
			logits = self.model_engine.forward(input_ids[i*mini_batch_size:(i+1)*mini_batch_size], attention_mask=attention_mask[i*mini_batch_size:(i+1)*mini_batch_size], use_cache=False).logits
			logits = logits[:, :-1]
			logits = logits[:, -logits_to_keep:]
			logps.append(gather_logps(logits + 1e-7, input_ids[i*mini_batch_size:(i+1)*mini_batch_size, -logits_to_keep:]))
			del logits
	...
	def playout(self, samples, is_first):
		...
		for sample in samples:
			messages = [
				...
				# {"role": "user", "content": sample['query'] + '\n' + "Show your work in <think> </think> tags. And return the final answer in <answer> </answer> tags with \\boxed{} wrapped, for example <answer> \\boxed{5} </answer>." },
			...
			all_text.append(text[:-11])

		if self.args.global_rank < 1:
			print(self.args.global_rank, all_text[0])
			# print(len(all_text))

		all_prompts = gather_object(all_text)
		if self.args.global_rank == 0:
